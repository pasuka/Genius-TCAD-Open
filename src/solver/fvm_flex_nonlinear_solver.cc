...
extern "C"
{
  ...
  static PetscErrorCode  __genius_petsc_snes_monitor (SNES, PetscInt its, PetscReal fnorm, void *ctx)
  {
    ...
    // convert void* to FVM_FlexNonlinearSolver*
    FVM_FlexNonlinearSolver * nonlinear_solver = (FVM_FlexNonlinearSolver *)ctx;
 ...
 */
FVM_FlexNonlinearSolver::FVM_FlexNonlinearSolver(SimulationSystem & system)
: FVM_FlexPDESolver(system), jacobian_matrix_first_assemble(false), Jac(0)
{
 ...
 */
void FVM_FlexNonlinearSolver::setup_nonlinear_data()
{
  ...
  Jac = new PetscMatrix<PetscScalar>(n_global_dofs, n_global_dofs, n_local_dofs, n_local_dofs);
  J = dynamic_cast<PetscMatrix<PetscScalar> *>(Jac)->mat();
 ...
 */
void FVM_FlexNonlinearSolver::dump_matrix_petsc(const Mat mat,const std::string &file) const
{
  PetscViewer viewer;
 ...
 */
void FVM_FlexNonlinearSolver::dump_matrix_asc(const Mat mat, const std::string &file) const
{
  PetscViewer viewer;
 ...
 */
void FVM_FlexNonlinearSolver::dump_matrix_triplet(const Mat mat, const std::string &file) const
{
  // serial only
 ...
 */
void FVM_FlexNonlinearSolver::dump_vector_petsc(const Vec vec, const std::string &file) const
{
  PetscViewer viewer;
  PetscViewerBinaryOpen(PETSC_COMM_WORLD, file.c_str(), FILE_MODE_WRITE, &viewer);
  VecView(vec, viewer);
 ...
 */
void FVM_FlexNonlinearSolver::load_vector_petsc(Vec vec, const std::string &file) const
{
  PetscViewer viewer;
...
std::vector<double> FVM_FlexNonlinearSolver::ksp_residual_history() const
{
  PetscInt na;
 ...
 */
void FVM_FlexNonlinearSolver::create_vector(Vec &vec)
{
  VecDuplicate(x, &vec);
}
 ...
 * destroy a vector
 */
void FVM_FlexNonlinearSolver::destroy_vector(Vec &vec)
{
 ...
 */
void FVM_FlexNonlinearSolver::clear_nonlinear_data()
{
  PetscErrorCode ierr;
  // free everything
 ...
 */
FVM_FlexNonlinearSolver::~FVM_FlexNonlinearSolver()
{
  delete Jac;
}
 ...
 */
void FVM_FlexNonlinearSolver::petsc_snes_monitor(PetscInt its, PetscReal fnorm)
{
 ...
 */
void FVM_FlexNonlinearSolver::petsc_snes_convergence_test(PetscInt its, PetscReal xnorm, PetscReal gnorm, PetscReal fnorm, SNESConvergedReason *reason)
{
 ...
 */
void FVM_FlexNonlinearSolver::petsc_ksp_convergence_test(PetscInt its, PetscReal rnorm, KSPConvergedReason* reason)
{
#if PETSC_VERSION_GE(3,5,0)
 ...
 */
void FVM_FlexNonlinearSolver::sens_line_search_pre_check(Vec , Vec , PetscBool *)
{
 ...
 */
void FVM_FlexNonlinearSolver::sens_line_search_post_check(Vec x, Vec y, Vec w, PetscBool *changed_y, PetscBool *changed_w)
{
...
void FVM_FlexNonlinearSolver::set_petsc_nonelinear_solver_type()
{
  int ierr = 0;
...
void FVM_FlexNonlinearSolver::set_petsc_preconditioner_type()
{
  int ierr = 0;
...
int FVM_FlexNonlinearSolver::set_petsc_option(const std::string &key, const std::string &value, bool has_prefix )
{
...
void FVM_FlexNonlinearSolver::snes_solve()
{
  START_LOG("sens_solve()", "FVM_FlexNonlinearSolver");

  // do snes solve
  SNESSolve ( snes, PETSC_NULL, x );

  
  STOP_LOG("sens_solve()", "FVM_FlexNonlinearSolver");
}



double FVM_FlexNonlinearSolver::condition_number_of_jacobian_matrix()
{
#ifdef HAVE_SLEPC
...
void FVM_FlexNonlinearSolver::eigen_value_of_jacobian_matrix(int n, int is, Vec Vrs, int il, Vec Vrl)
{
  PetscErrorCode ierr;
